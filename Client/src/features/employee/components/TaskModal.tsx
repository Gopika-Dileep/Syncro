...
export default function TaskModal({ isOpen, onClose, onSubmit, initialData, isEditing = false, isSubmitting = false, members = [] }: TaskModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            if (initialData) {
                ...
                setAssignTo(initialData.assign_to || "");
            ...
            <div className="bg-[#f8f9fa] rounded-[28px] w-full max-w-md shadow-2xl relative z-[1060] flex flex-col max-h-[95vh] animate-in fade-in zoom-in duration-200 overflow-hidden border border-white/20">
                ...
                <div className="px-7 py-5 overflow-y-auto flex-1 custom-scrollbar">
                    <form id="task-form" onSubmit={handleSubmit} className="space-y-4">
                        ...
                        {/* Title */}
                        <div>
                            <label className="block text-[12px] font-bold text-[#1f2124] uppercase tracking-wider mb-2 ml-1 opacity-60">Task Title *</label>
                            <input
                                type="text"
                                required
                                value={title}
