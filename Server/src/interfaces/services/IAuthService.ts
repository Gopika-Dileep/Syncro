export interface IAuthService {
    ...
    verifyOtp(email:string,otp:string):Promise<{accessToken:string, refreshToken:string,role:string}>
    resendOtp(email: string): Promise<{ message: string }>;
    login(email:string,password:string):Promise<{ accessToken:string ,refreshToken:string,role:string}>
    refresh(refreshToken:string):Promise<{accessToken:string , role:string}>
