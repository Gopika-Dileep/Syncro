...
export class BaseRepository<T extends Document> implements IBaseRepository<T> {
  ...
  async findOne(filter: Record<string, any>, options?: mongoose.QueryOptions): Promise<T | null> {
    return await this._model.findOne(filter, null, options).exec();
  }

  async find(filter: Record<string, any>, options?: mongoose.QueryOptions): Promise<T[]> {
    return await this._model.find(filter, null, options).exec();
  }

  async updateById(id: string, update: Record<string, any>, options?: mongoose.QueryOptions): Promise<T | null> {
    return await this._model.findByIdAndUpdate(id, update, { new: true, ...options }).exec();
  }

  async updateOne(filter: Record<string, any>, update: Record<string, any>, options?: mongoose.QueryOptions): Promise<T | null> {
